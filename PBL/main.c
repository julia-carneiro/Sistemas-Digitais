...
void *botao(){
  ...
  return (-1);
  ptr_BT = (unsigned int *) (LW_virtual + KEYS_BASE);
  while(1){
    if(*ptr_BT== 13){
      printf("%d\n",parar);
      parar = !parar;
...
int main() {
  ...
  if(pthread_create(&thread_botao, NULL, botao, NULL)){
      printf("Erro ao criar thread botao.");
  }
  
  if(pthread_create(&thread_mouse, NULL, movimento_mouse, NULL)){
    printf("Erro ao criar thread mouse.");
  }
    ...
    if(iniciou){
      ...
      while(vida && gasolina){
        ...
        if(parar){// pause
          ...
          pausar();
          despausou = 1; //isso aqui tem que limpar certo ainda 
        }
      }
      perdeu = 1;
      iniciou = 0;
  ...
  pthread_join(thread_botao,NULL);
